      ...
      {view === "assets" && (
        <div className="workspace-stack">
          ...
          <section className="authority-grid">
            ...
            <div className="sync-contract"><p className="eyebrow">One-write contract</p><h2>更新一次，所有视图同步</h2><div className="sync-flow"><span>Canonical index</span><i>→</i><span>Atomic writer</span><i>→</i><span>Generated snapshot</span><i>→</i><span>Browser views</span></div><dl><div><dt>Structured state</dt><dd>memory-proactive-agent-research/research-index.yaml</dd></div><div><dt>Append-only changes</dt><dd>research-events.jsonl</dd></div><div><dt>Settlement writer</dt><dd>scripts/settle-research-event.mjs</dd></div><div><dt>Source revision</dt><dd title={researchIndex.sourceRevision}>{researchIndex.sourceRevision.slice(0, 21)}…</dd></div><div><dt>Dashboard adapter</dt><dd>npm run sync:data</dd></div><div><dt>Evidence boundary</dt><dd>External Review 只能 pressure，不能 support</dd></div></dl></div>
          </section>
